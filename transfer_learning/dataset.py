...
class EmulatorDataset(Dataset):
    def __init__(self, input_file, exclude_site=None):
        self.frame = get_dataframe(input_file, exclude_site)
        self.n_params = 20
        self.out_dims = 884
    ...
    def __getitem__(self, idx):
        ...
        output_vals = torch.tensor(
            np.float32(row[(self.n_params + 2) :]),
            dtype=torch.float32,
        )
