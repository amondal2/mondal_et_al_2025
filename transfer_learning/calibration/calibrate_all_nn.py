"""
Nearest neighbors calibration for all sites using transfer learning.
"""
...
def calculate_loss_for_site(emulator_outputs, reference_data, site, dims, exclude_site):
    loss_functions = {
...
def generate_prevalence_plots(
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/nearest_neighbors/{site}/prevalence_{idx}.pdf"
...
def generate_incidence_plots(
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/nearest_neighbors/{site}/incidence_{idx}.pdf"
        plt.savefig(os.path.expanduser(fig_path))
        plt.clf()
    return True
...
def generate_parasitemia_gametocytemia_plots(
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/nearest_neighbors/{site}/parasitemia_{idx}.pdf"
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/nearest_neighbors/{site}/gametocytemia_{idx}.pdf"
        fig.supxlabel("per uL")
        fig.supylabel("Fraction of Population")
        plt.setp(axs, ylim=(0, 1))
        plt.savefig(os.path.expanduser(fig_path))
...
if __name__ == "__main__":
    ...
    for exclude_site in params.sites:
        ...
        min_loss_df.to_csv(
            os.path.expanduser(
                f"~/EMOD-calibration/transfer_learning/calibration/data/exclude_{exclude_site}/nearest_neighbors_joint_loss_min.csv"
            )
        )
