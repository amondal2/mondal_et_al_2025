"""
Likelihood calibration for all sites using transfer learning.
"""
...
def calculate_sim_likelihood_prevalence(
    ...
    sim_pop = 1000
    site_pop = site_metadata[site]["population_by_age_bin"]
    sim_observations = pd.Series(sim_outputs) * sim_pop
    reference_observations = pd.Series(reference_data) * site_pop
    sim_df = pd.DataFrame({"Trials": sim_pop, "Observations": sim_observations})
    reference_df = pd.DataFrame(
        {"Trials": site_pop, "Observations": reference_observations}
    )
    likelihood = beta_binomial_likelihood(sim_df, reference_df)
    return {"ll": likelihood, "penalty_type": "beta-binomial"}


def calculate_sim_likelihood_incidence(
    sim_outputs=None,
    reference_data=None,
    site=None,
    dims=None,
    ...
    likelihood = gamma_poisson_likelihood(sim_df, reference_df)
    return {"ll": likelihood, "penalty_type": "gamma-poisson"}
...
def format_sim_data_garki(sim_outputs=None, months=None, dims=None):
    return generate_sim_trial_obs_df(
        dims=dims,
        output_key="Parasitemia_2",
        channel_name="Smeared PfPR by Parasitemia and Age Bin",
        sim_outputs=sim_outputs,
...
def generate_sim_trial_obs_df(dims, output_key, channel_name, sim_outputs, months):
    sim_pop = 1000
    n_month = 12
    n_age = dims[output_key]["n_age"]
    n_density = dims[output_key]["n_density"]
...
def format_sim_data_burkinafaso(sim_outputs=None, months=None, dims=None):
    # Data are ordered by (month, age)
    # Create data frame and rebin/index like reference data
    parasitemia_df = generate_sim_trial_obs_df(
        dims=dims,
        output_key="Parasitemia_1",
        channel_name="Smeared PfPR by Parasitemia and Age Bin",
        sim_outputs=sim_outputs,
    ...
    gametocytemia_df = generate_sim_trial_obs_df(
        dims=dims,
        output_key="Gametocytemia_1",
        channel_name="Smeared PfPR by Gametocytemia and Age Bin",
        sim_outputs=sim_outputs,
...
def calculate_sim_likelihood_parasitemia(
    sim_outputs=None, reference_data=None, site=None, dims=None
):
...
def calculate_sim_likelihood_parasitemia_gametocytemia(
    sim_outputs=None, reference_data=None, site=None, dims=None
):
...
def calculate_likelihood_for_site(
    emulator_outputs, reference_data, site, dims, exclude_site
):
    ll_functions = {
    ...
    ll = ll_functions[site]

    if site == exclude_site:
        return {"ll": 0.0, "penalty_type": "NA"}

    return ll(
        emulator_outputs=dict(
            zip(outputs_for_site, [emulator_outputs[o] for o in outputs_for_site])
        ),
...
def calculate_sim_likelihood_for_site(sim_means, reference_data, site, dims):
    ll_functions = {
        "namawala_1991": calculate_sim_likelihood_prevalence,
        "ndiop_1993": calculate_sim_likelihood_incidence,
        "dielmo_1990": calculate_sim_likelihood_incidence,
        "matsari_1970": calculate_sim_likelihood_parasitemia,
        "rafin_marke_1970": calculate_sim_likelihood_parasitemia,
        "sugungum_1970": calculate_sim_likelihood_parasitemia,
        "dapelogo_2007": calculate_sim_likelihood_parasitemia_gametocytemia,
        "laye_2007": calculate_sim_likelihood_parasitemia_gametocytemia,
    }
    ll = ll_functions[site]
    return ll(
...
def find_max_joint_likelihood(likelihood_df):
    ...
    for df in dfs:
        ...
        likelihood = np.sum(df["likelihood"])
        sim_likelihood = np.sum(df["sim_likelihood"])
        row = pd.concat(
            [
                input_params,
                pd.Series(
                    {"joint_likelihood": likelihood, "sim_likelihood": sim_likelihood}
                ),
            ]
        )
    ...
    return joint_likelihood_df.sort_values(by=["sim_likelihood"], ascending=False).head(
        10
    )


def generate_prevalence_plots(
    site, likelihood_df, max_likelihood_df, mlp, min_values, max_values, exclude_site
):
    site_df = likelihood_df[
        likelihood_df.Site == site_tokens[site].iloc[0]
    ].reset_index(drop=True)
    # Eval emulator
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/likelihood/{site}/prevalence_{idx}.pdf"
...
def generate_incidence_plots(
    ...
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/likelihood/{site}/incidence_{idx}.pdf"
...
def generate_parasitemia_gametocytemia_plots(
    ...
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/likelihood/{site}/parasitemia_{idx}.pdf"
    ...
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        for i, month in enumerate(months_for_site):
            ...
            curr_month = sorted(months_for_site)[i]
            axs[0, i].set_title(calendar.month_name[curr_month])
            for j in range(n_row):
                curr_age = reference_age_labels[j]
                reference_values = reference.loc[
        ...
        fig_path = f"~/EMOD-calibration/transfer_learning/calibration/plots/exclude_{exclude_site}/joint/likelihood/{site}/gametocytemia_{idx}.pdf"
        fig.supxlabel("per uL")
        fig.supylabel("Fraction of Population")
        plt.setp(axs, ylim=(0, 1))
        plt.savefig(os.path.expanduser(fig_path))
    ...
    for exclude_site in params.sites:
        ...
        with torch.no_grad():
            for df in dfs:
                ...
                likelihood = calculate_likelihood_for_site(
                    emulator_outputs=outputs,
                        ...
                        pd.Series(
                            {
                                ...
                                "penalty_type": likelihood["penalty_type"],
        ...
        likelihood_df = pd.concat(likelihood_rows, axis=1).transpose()
        max_likelihood_df = find_max_joint_likelihood(likelihood_df)
        max_likelihood_df.to_csv(
            os.path.expanduser(
                f"~/EMOD-calibration/transfer_learning/calibration/data/exclude_{exclude_site}/likelihood_joint_max.csv"
            )
        )
