...
def generate_prevalence_plots(site, loss_df, min_loss_df, mlp, min_values, max_values):
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/sgd/{site}/prevalence_{idx}.pdf"
...
def generate_incidence_plots(site, loss_df, min_loss_df, mlp, min_values, max_values):
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = (
            f"~/EMOD-calibration/calibration/plots/joint/sgd/{site}/incidence_{idx}.pdf"
        )
        plt.savefig(os.path.expanduser(fig_path))
        plt.clf()
    return True
...
def generate_parasitemia_gametocytemia_plots(
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/sgd/{site}/parasitemia_{idx}.pdf"
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/sgd/{site}/gametocytemia_{idx}.pdf"
        fig.supxlabel("per uL")
        fig.supylabel("Fraction of Population")
        plt.setp(axs, ylim=(0, 1))
        plt.savefig(os.path.expanduser(fig_path))
...
if __name__ == "__main__":
    ...
    path = os.path.expanduser(
        "~/EMOD-calibration/emulator/scaled_df_sgd_calibration.pkl"
    )
    ...
    emulator_loss_df = pd.read_csv(
        "~/EMOD-calibration/calibration/data/sgd_joint_loss_min.csv", index_col=0
    )
    min_loss_df = pd.merge(min_loss_df, emulator_loss_df).sort_values(
