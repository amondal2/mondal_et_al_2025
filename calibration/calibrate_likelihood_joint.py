...
def calculate_likelihood_for_site(emulator_outputs, reference_data, site, dims):
    ll_functions = {
...
def find_max_joint_likelihood(likelihood_df):
    ...
    return joint_likelihood_df.sort_values(
        by=["joint_likelihood"], ascending=False
    ).head(10)


def generate_prevalence_plots(
    site, likelihood_df, max_likelihood_df, mlp, min_values, max_values
):
    ...
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        inputs = torch.tensor(
            site_df_row.iloc[0:8].values.astype("float"),
            dtype=torch.float32,
        ).reshape(1, 8)
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/likelihood/{site}/prevalence_{idx}.pdf"
...
def generate_incidence_plots(
    ...
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/likelihood/{site}/incidence_{idx}.pdf"
        plt.savefig(os.path.expanduser(fig_path))
        plt.clf()
    return True
...
def generate_parasitemia_gametocytemia_plots(
    ...
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        inputs = torch.tensor(
            site_df_row.iloc[0:8].values.astype("float"), dtype=torch.float32
        ).reshape(1, 8)
        inputs = normalize(inputs, min_values, max_values, num_features=7)
        outputs = mlp(inputs)
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/likelihood/{site}/parasitemia_{idx}.pdf"
    ...
    for idx, row in max_likelihood_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/likelihood/{site}/gametocytemia_{idx}.pdf"
        fig.supxlabel("per uL")
        fig.supylabel("Fraction of Population")
        plt.setp(axs, ylim=(0, 1))
        plt.savefig(os.path.expanduser(fig_path))
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for df in dfs:
            ...
            sim_sd = df.iloc[:, 9:].std().rename("std_{}".format)
    ...
    max_likelihood_df.to_csv(
        "~/EMOD-calibration/calibration/data/likelihood_joint_max.csv"
    )
    generate_prevalence_plots(
        "namawala_1991", likelihood_df, max_likelihood_df, mlp, min_values, max_values
    )
