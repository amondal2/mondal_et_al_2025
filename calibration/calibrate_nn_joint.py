...
def generate_emulator_df(dims, output_key, channel_name, emulator_outputs, months):
    ...
    emulator_df["Counts"] = emulator_outputs[output_key][0]
...
def calculate_loss_for_site(emulator_outputs, reference_data, site, dims):
    loss_functions = {
...
def generate_prevalence_plots(site, loss_df, min_loss_df, mlp, min_values, max_values):
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/nearest_neighbors/{site}/prevalence_{idx}.pdf"
        plt.savefig(os.path.expanduser(fig_path))
        plt.clf()
    return True


def generate_incidence_plots(site, loss_df, min_loss_df, mlp, min_values, max_values):
    site_df = loss_df[loss_df.Site == site_tokens[site].iloc[0]].reset_index(drop=True)
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/nearest_neighbors/{site}/incidence_{idx}.pdf"
        plt.savefig(os.path.expanduser(fig_path))
        plt.clf()
    return True
...
def generate_parasitemia_gametocytemia_plots(
    site, loss_df, min_loss_df, mlp, min_values, max_values
):
    # TODO cleanup
    site_df = loss_df[loss_df.Site == site_tokens[site].iloc[0]].reset_index(drop=True)
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/nearest_neighbors/{site}/parasitemia_{idx}.pdf"
    ...
    for idx, row in min_loss_df.reset_index(drop=True).iterrows():
        ...
        fig_path = f"~/EMOD-calibration/calibration/plots/joint/nearest_neighbors/{site}/gametocytemia_{idx}.pdf"
        fig.supxlabel("per uL")
        fig.supylabel("Fraction of Population")
        plt.setp(axs, ylim=(0, 1))
        plt.savefig(os.path.expanduser(fig_path))
...
def generate_parasitemia_plots(site, loss_df, min_loss_df, mlp, min_values, max_values):
    site_df = loss_df[loss_df.Site == site_tokens[site].iloc[0]].reset_index(drop=True)
    months_for_site = site_metadata[site]["months"]
...
def find_min_joint_loss(loss_df):
    ...
    for df in dfs:
        ...
        emulator_loss = np.sum(df["loss"])
        sim_loss = np.sum(df["sim_loss"])
        row = pd.concat(
            [
                input_params,
                pd.Series({"joint_loss": emulator_loss, "joint_sim_loss": sim_loss}),
            ]
        )
        joint_loss_rows.append(row)
    joint_loss_df = pd.concat(joint_loss_rows, axis=1).transpose()
    # Bottom 10 loss values
    return joint_loss_df.sort_values(by=["joint_loss"], ascending=True).head(10)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for df in dfs:
            ...
            row = pd.concat(
                [
                    input_params,
                    pd.Series({"loss": loss, "sim_loss": sim_loss}),
                    sim_means,
                ]
            )
    ...
    min_loss_df.to_csv(
        "~/EMOD-calibration/calibration/data/nearest_neighbors_joint_loss_min.csv"
    )

    generate_prevalence_plots(
        "namawala_1991", loss_df, min_loss_df, mlp, min_values, max_values
    )
