...
def load_data(batch_size=32, site=None):
...
def train_mlp(
    config, loss_function, device="cpu", dims=None, max_values=None, min_values=None
):
...
if __name__ == "__main__":
    ...
    test_loss_individual = {k: v / num_batches for k, v in test_loss_individual.items()}
    print("Best trial test set loss: {}".format(test_loss_total))
    best_result.metrics["test_set_loss"] = test_loss_total

    # Write everything to disk
    timestamp = int(time.time())
    config_path = f"~/EMOD-calibration/emulator/multisite/configs/best_result_config_multisite_{timestamp}.json"
    metrics_path = f"~/EMOD-calibration/emulator/multisite/metrics/best_result_metrics_multisite_{timestamp}.json"
    ...
    model_path = f"~/EMOD-calibration/emulator/multisite/trained_models/model_optim_multisite_{timestamp}"
    torch.save(best_trained_model.state_dict(), os.path.expanduser(model_path))
